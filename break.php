...
$sql2 = mysql_query("SELECT * FROM log WHERE user_name='$username' AND timestamp=CURDATE()"); // query the person
  ...
  if ($existCount == 1) { // evaluate the count
		$sql22 = mysql_query("UPDATE log set total_break = total_break + $breakRange WHERE user_name='$username' AND timestamp=CURDATE()");
    }
...
function move() {
    ...
    var width = 1;
    var id = setInterval(frame, breakRange*10);
    function frame() {
        if (width >= 100) {
            clearInterval(id);
        } else {
  ...
  <div id="wrapper">
    ...
    <section id="sidebar">
      ...
      <section id="intro">
        ...
        <!-- Title -->
        <header>
          <h2>BREAK REMAINING</h2>
          <p>How much time you have left for your break</p>
       </br>
...
<div id="myProgress">
    <div id="myBar"></div>
     ...
     <!-- <div id="sessions">
        <h3>Sessions Remaining:  <input type="submit" name="commit" value="#" onclick="getSession()"></h3>
...
CountDown.Start(worktimeB * 600);</script>
   ...
   <script>
      function getSession() {
         $.getJSON("./accounts.php", function(json) {
          //console.log(json[0].accounts);
          var sessions = json[0].accounts[0].sessions; 
          alert(sessions);
          });
        }
  </script>
