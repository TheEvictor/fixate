...
$sql1 = mysql_query("SELECT timestamp, total_break FROM log WHERE user_name='$username' ORDER BY timestamp ASC LIMIT 5");
$cnt = mysql_num_rows($sql1); // count the output amount
if ($cnt > 0) {
	while($row = mysql_fetch_array($sql1)){ 
             $timestamp = $row["timestamp"];
			 $total_break = $row["total_break"];
             $dataArray[$timestamp]=$total_break;
	}
}
$graph->addData($dataArray);
$graph->setTitle("Recent break activity");
